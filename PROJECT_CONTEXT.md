# BhuRakshak â€” Project Context

> SIH26001 Â· AI-Based Early Warning and Landslide Risk Monitoring System in NER
...
BhuRakshak (à¤­à¥‚à¤°à¤•à¥à¤·à¤• â€” "Earth-Guardian") is a 4-layer landslide early warning
system. Each layer answers one question, then they fuse:

| Layer | Question | Engine | Output |
|---|---|---|---|
| **A â€” Susceptibility** (static) | WHERE can landslides happen? | XGBoost / LightGBM on 24 terrain + geology + land-cover features per 30 m grid cell, **leave-one-district-out (LODO) spatial CV** | Static 0â€“100 score per cell â†’ 5 GSI-compatible classes |
| **B â€” Hazard Nowcast** (dynamic) | WHEN is it dangerous? | LightGBM + isotonic calibration **fused** with interpretable intensityâ€“duration (I-D) thresholds; 72h rainfall forecast features; **hysteresis** (escalate after 2 ticks, de-escalate after 3) | Hazard level L0â€“L4 per response zone, now + 24/48/72 h |
| **C â€” Deformation** (slow-motion) | IS THE SLOPE MOVING? | Sentinel-1 PSInSAR LOS velocity â†’ robust z-score â†’ DBSCAN creep clusters | Active creep zones; auto +1 hazard tier upgrade |
| **D â€” Exposure & Response** | WHO / WHAT is in the way? | Hazard Ã— population (WorldPop proxy) Ã— road criticality (NH > SH > village) Ã— isolation score | Ranked response-priority queue; blocked-road prediction + NetworkX A\* detours |

**Fusion rule.** `hazard_level = max(threshold_tier, calibrated_ML_tier)`.
**Anti-flapping hysteresis.** Escalate only after 2 consecutive ticks â‰¥ candidate;
de-escalate only after 3 consecutive ticks below `current âˆ’ 1`.
**Stack rationale (defensible Q&A):** FastAPI async + pydantic v2 for OpenAPI +
WebSocket live alerts Â· PostgreSQL 15 + PostGIS + TimescaleDB in a single
`timescale/timescaledb-ha` image Â· Martin (Rust) for MVT serving Â·
JWT access + rotating refresh tokens with **family-reuse detection** Â· Celery
beat for crash-safe ingestion Â· Next.js 14 + ECharts for SHAP waterfalls Â·
Vite PWA + Workbox + Dexie for offline-first field reporting with idempotent
batch sync by client UUID Â· slowapi for endpoint brute-force protection.
...
```
   DATA â”€â”€â”€ Open-Meteo Â· GSI Bhukosh Â· NASA COOLR Â· Copernicus GLO-30 Â·
            Sentinel-1 / LiCSAR Â· OSM Â· WorldPop Â· IoT MQTT sensors
     â”‚
   INGEST â”€â”€â”€ Celery beat: rainfall poll 15 m Â· risk recompute 15 m Â·
              seismic 1 h Â· satellite-ETL daily Â· MQTT bridge 24Ã—7
     â”‚
   INTELLIGENCE â”€â”€â”€ ml/
     â”œâ”€ ingest/{dem,inventory,weather}.py        (synthetic + GLO-30 hooks)
     â”œâ”€ models/{susceptibility,hazard,deformation,backtest}.py
     â””â”€ registry/registry.py                     (artifact_meta + model cards)
     â”‚
   BACKEND â”€â”€â”€ apps/api (FastAPI)
     â”œâ”€ REST   /api/v1/{auth,zones,reports,alerts,roads,demo,analytics,ingest,briefing}
     â”œâ”€ WS     /ws/live                          (Redis pub/sub fan-out)
     â”œâ”€ Alert  Hysteresis + 8-language i18n templates + ack workflow
     â””â”€ Sync   Idempotent UUID + 50 m / 1 h proximity dedupe
     â”‚
   STORAGE â”€â”€â”€ PostgreSQL 15 + PostGIS + TimescaleDB (4 hypertables:
              rainfall_obs, sensor_readings, displacement_series,
              risk_snapshots) Â· Redis (queues + pub/sub) Â· MinIO (media)
              Martin (vector tiles at /:table/:z/:x/:y.pbf)
     â”‚
   FRONT â”€â”€â”€ apps/dashboard (Next.js 14 Â· TS Â· Tailwind Â· MapLibre GL 3D + ECharts SHAP)
             apps/field-pwa (Vite PWA Â· Workbox Â· Dexie Â· 8-language UI)
     â”‚
   ALERT CHANNELS â”€â”€â”€ Push | SMS (stub) | IVR (stub) | Siren (stub)
                       â† stubbed but adapters' channel list stored on the Alert row
```

## Current State (updated 2026-09-02 — Model A v2 + Android rebuild; see GAP_ANALYSIS.md for the full matrix)

Additions on top of the 2026-08-29 audit below:

- **Model A v2 (Tier-2 micro-susceptibility)** — `ml/ingest/dem_real.py` (real
  ~35 m terrarium DEM for all 5 AOIs, key-less), `ml/features/micro_terrain.py`
  (13 derivatives incl. D8 routing TWI/SPI/valley distance), and
  `ml/models/micro_susceptibility.py` (block-level labels vs GLC noise,
  strict LODO leaderboard: booster vs slope/elev logistic baselines vs the
  deployed slope-dominant physical index; the measured leaderboard ships in
  `ml/artifacts/model_a_micro_metrics.json`). Served by
  `GET /api/v1/analytics/micro-heatmap` and applied to the DB via
  `POST /api/v1/analytics/micro-heatmap/refresh-susceptibility` (536 zones
  refreshed live; susc_mean/p90 are now real terrain statistics, not hashes).
- **Android app (apps/android) rebuilt end-to-end** — the committed source
  could not compile (`package in.` is a Kotlin keyword) and its screens were
  stubs. Now: buildable project (gradle wrapper 8.7, AGP 8.5.2, Kotlin 2.0.20,
  KSP Room, res/ manifest fix), package `com.bhrakshak.field`, and real
  screens — risk-now w/ offline cache, I'M-SAFE check-in (Room), report
  composer with camera capture + Model V AI pre-screen, safe-route canvas
  rendering of the pathway model, rain gauge with I-D breach status, alert
  history, WS push notifications (foreground service), connectivity-triggered
  sync. Debug APK assembles (8.4 MB).
- **Storm injector idempotency** — same-hour re-run used to 422 on the
  rainfall_obs PK; now an upsert, re-verified live.
- Tests: 119 backend+ML green; dashboard `tsc && next build` clean; field PWA
  `tsc && vite build` clean.

The 2026-08-29 audit below is retained for history; GAP_ANALYSIS.md is the
current matrix.

Code present in repository (`887a6d1` â€” `master`):
113 tracked files, â‰ˆ **15.4 K LOC** total (â‰ˆ 4.0 K Python, â‰ˆ 2.4 K TS/TSX,
plus JSON, YAML, CSS, Markdown). Architecture is intentionally modular with
hard, named service boundaries (api â‡„ worker â‡„ ml â‡„ front).

### âœ… Built & Working

**Backend (FastAPI Â· async SQLAlchemy 2.0 Â· pydantic v2 Â· asyncpg):**
- 13 routers mounted under `/api/v1` + 1 WebSocket at `/ws/live`. Endpoints
  cover auth (4), zones (2), reports (4), alerts (3), roads (2), demo (2),
  analytics (4), briefing (1), ingest (1) â€” see "API surface" below.
...
- **Risk-engine service** (`app/services/risk_engine.py`) implementing:
  intensityâ€“duration thresholds per susceptibility band Â· hysteresis
  (2-tick escalate / 3-tick de-escalate) Â· multi-horizon snapshots
  (now / f24 / f48 / f72) Â· i18n alert template lookup with fallback Â·
  Redis pub/sub broadcast consumed by WS.
- **Multi-hazard + response-priority service** (`app/services/priority.py`):
  `flood_index()` (flash-flood tier 0â€“4) + `isolation_score()` (deterministic
  proxy until OSM centrality lands) + `priority_rows()` ranked queue
  (hazard Ã— exposure Ã— vulnerability) with human-readable reason chips and
...
**Worker (Celery Â· Redis broker/result Â· Flower ops UI):**
- Celery app (`apps/worker/worker/celery_app.py`) with **beat schedule**:
  rainfall poll every 15 m Â· risk recompute every 15 m (+ 30 s countdown
  to dedupe with rainfall) Â· seismic poll every 1 h Â· satellite ETL daily.
...
- **Task implementations**: `tasks/ingest.py` (Open-Meteo archive/forecast,
  Kohlerâ€“Linsley effective rainfall, half-life 48 h, deterministic synthetic
  fallback), `tasks/risk.py` (calls `evaluate_all_zones`), `tasks/seismic.py`
  (USGS FDSN, M â‰¥ 4 within 100 km of pilot centroids in last 7 d, trigger
  flag persisted).
- **Flower** ops UI on `:5555`.

**Database (alembic Â· PostGIS Â· TimescaleDB Â· asyncpg):**
...
- **4 TimescaleDB hypertables** (`risk_snapshots`, `rainfall_obs`,
  `displacement_series`, `sensor_readings`) â€” auto-chunked on `ts`.
  Retention policy 400 days on `rainfall_obs`.
- Foreign-key cascades wired (`users â†’ refresh_tokens`, `zones â†’
  risk_cells`, `risk_snapshots`, `rainfall_obs`; `alerts.zone_id â†’ zones`).
...
**Frontend (Next.js 14 dashboard Â· TS Â· Tailwind Â· MapLibre GL Â· ECharts):**
- **Command Center** (`/`): 3-D MapLibre map (CARTO dark raster + DEM
  hillshade), Martin vector-tile layers for `zones`, `risk_cells`,
  `road_status`, `citizen_reports`. Sub-second hover highlight, click â†’
  ...
  Layer toggle rail + district fly-to + demo storm "â›ˆ Inject Monsoon Cell"
  judge button.
- **Analytics** (`/analytics`): P OD/FAR/CSI by warning-level chart, lead-time
  ...
  TIME** callout when â‰¥ 24 h & level â‰¥ 3.
...
**Field PWA (Vite Â· Workbox Â· Dexie Â· TypeScript):**
...
**ML pipeline (`ml/`):**
- Synthetic-data fallback everywhere â€” pipeline is **fully offline-safe**.
- `ml/ingest/{dem,inventory,weather}.py` (Copernicus GLO-30 hook + GSI
  Bhukosh / NASA COOLR loaders with fallback).
- `ml/models/susceptibility.py` (XGBoost w/ LightGBM sklearn fallback Â·
  LODO spatial CV on 16 000 synthetic cells across 4 districts Â·
  permutation importances).
- `ml/models/hazard.py` (LightGBM w/ fallback Â· temporal split
  â‰¤2019 / 20â€“22 / 23â€“24 Â· isotonic calibration).
- `ml/models/deformation.py` (robust-z on synthetic PSInSAR velocity Â·
  DBSCAN clustering).
- `ml/models/backtest.py` POD/FAR/CSI per level + lead-time histogram â†’
...
- `infra/martin/config.yml` exposes 4 tables: `risk_cells`, `zones`,
  ...
  cover NER.
- `infra/mosquitto/mosquitto.conf` â€” anonymous dev listener on `:1883`,
...
- `apps/api/tests/test_api.py` covers `health`, `auth/login` (bad creds),
  ...
  detection**, RBAC `citizenâ†’verify_report` denial, idempotent
  `reports/sync`, RBAC `citizenâ†’demo_storm` denial, `zones` requires
  auth.

### âš ï¸ Built but on Seeded / Synthetic Data
...
- **Backtest fixture** POD/FAR/CSI numbers are pre-baked with deterministic
  random seeds in `ml/models/backtest.py` â€” these are **the numbers shown on
  the analytics page** (POD 0.78 / FAR 0.31 / CSI 0.58 for L2 etc.).
- **Noney-2022 timeline** is hand-coded in the fixture
  (`MN-NON-002`, `2022-06-30`, 58 fatalities) â€” single backtest anchor.
- **PWA sync logins** as the demo citizen (hard-coded credentials) for the
  one-tap sync flow.

### âŒ Not Yet Built (the Winning Gaps)

| # | Gap | Where it lands | Why it matters |
|---|---|---|---|
| 1 | **Real ML Models A + B** producing scored cells per real-feeling dataset | `ml/models/susceptibility.py`, `ml/models/hazard.py` | Judges ask "is the ML real?" â€” right now it's threshold-based + SHAP-shaped placeholders |
...
| 4 | **Full PWA wiring** to backend with proper login (vs hard-coded demo citizen) | `apps/field-pwa/src/db.ts` `syncQueue` currently logs in as citizen@â€¦ | Real field-official testing |
...
| 10 | **Hardware sensor integration** (ESP32 firmware for `sensors/#` topic) | repo `/sensors/` firmware does not exist | "We built the box too" â€” the MoE-grade move |

## Tech Stack

| Layer | Tech | Why (defensible) |
|---|---|---|
| API | FastAPI 0.115 async Â· pydantic v2 Â· SQLAlchemy 2.0 async Â· asyncpg | Same language as ML Â· WebSocket for live alerts Â· auto-OpenAPI for judges |
| Worker | Celery 5.4 Â· Redis 7 Â· paho-mqtt 2.x Â· httpx Â· Flower 2.x | Crash-safe scheduled ingest Â· ops visibility |
| Datastore | PostgreSQL 15 + PostGIS + TimescaleDB in one `timescale/timescaledb-ha` image Â· `pgcrypto` | Standard geospatial + time-series without a second DB |
| Vector tiles | Martin (Rust) Â· GeoJSON â†’ MVT at `/:table/:z/:x/:y.pbf` | What real geo teams self-host; MapLibre-ready |
| Cache / queue / fanout | Redis 7 (queues, results, pub/sub) | Single 6360â†’6380 mapped service |
...
| Dashboard | Next.js 14.2 Â· TS Â· Tailwind 3.4 Â· MapLibre GL 4.7.1 (3-D pitch + hillshade) Â· ECharts 5 (SHAP / lead-time) Â· Zustand 4 Â· Lucide icons | Type-safe pre-demo; ECharts lets judges interrogate SHAP |
| PWA | Vite 5 Â· React 18 Â· TS Â· Workbox (NetworkFirst /api/*) Â· Dexie 4 (IndexedDB) | Offline-first field reporting |
| ML | numpy Â· pandas Â· geopandas Â· shapely Â· scikit-learn Â· LightGBM 4 Â· XGBoost (optional) Â· `ml/Makefile` offline pipeline | LODO spatial CV â€” gold standard for spatial ML |
| Infra | Docker Compose (10 services) Â· Martin config Â· Mosquitto 2 Â· backend.Dockerfile (python 3.11-slim) | Fits a 16 GB laptop (PG capped 1.5 GB) |
...
| Zone list + dossier (rain series Â· sensors Â· reports Â· alerts Â· drivers Â· history Â· flood Â· isolation) | `apps/api/app/api/v1/zones.py` |
| Reports: create Â· batch sync (idempotent) Â· list Â· verify | `apps/api/app/api/v1/reports.py` |
| Alerts: list Â· ack Â· preview-fire | `apps/api/app/api/v1/alerts.py` |
| Roads: status Â· detour (NetworkX A\*) | `apps/api/app/api/v1/roads.py` |
| Demo: inject-rainfall-storm Â· replay-event | `apps/api/app/api/v1/demo.py` |
| Analytics: KPIs Â· backtest fixture Â· model registry Â· response priority | `apps/api/app/api/v1/analytics.py` |
...
| Priority engine (flood Â· isolation Â· ranked queue) | `apps/api/app/services/priority.py` |
...
| Seed (hex grid Â· users Â· roads Â· i18n Â· model-registry entry) | `scripts/seed.py` |
| Realistic demo state (reports Â· blocked road Â· sensor fleet) | `scripts/seed_realistic.py` |
| Storm injector (admin login â†’ real pipeline â†’ L2+) | `demo/storm_injector.py` |
| Sensor simulator (MQTT â†’ DB, HTTP fallback) | `demo/sensor_simulator.py` |
| Backtest fixture | `demo/backtest_fixture.json` (regenerated by `ml/models/backtest.py`) |
| ML: susceptibility Â· hazard Â· deformation Â· backtest | `ml/models/{susceptibility,hazard,deformation,backtest}.py` |
| ML ingest: DEM (Copernicus GLO-30) Â· inventory (GSI / COOLR) Â· weather (Open-Meteo) | `ml/ingest/{dem,inventory,weather}.py` |
| Worker / Celery boot + beat schedule | `apps/worker/worker/celery_app.py` |
| MQTT bridge | `apps/worker/worker/mqtt_bridge.py` |
| Periodic tasks: rainfall Â· risk Â· seismic Â· satellite ETL | `apps/worker/worker/tasks/{ingest,risk,seismic}.py` |
...
| Dashboard analytics page (P OD/FAR/CSI Â· lead-time Â· LODO Â· I-D Â· registry + Noney replay) | `apps/dashboard/src/app/analytics/page.tsx` |
| Dashboard operations page (queue + alert console) | `apps/dashboard/src/app/operations/page.tsx` |
| Dashboard layout (KPI bar Â· top nav Â· ticker) | `apps/dashboard/src/app/layout.tsx` |
...
| Make targets (`up Â· migrate Â· seed Â· demo Â· test Â· data`) | `Makefile` |

## How to Run Locally

```bash
cp .env.example .env                  # use Postgres on host port 5433 (deliberate)
make up                               # 10 services: pg Â· redis Â· mqtt Â· minio Â· martin Â· api Â· worker Â· beat Â· flower Â· dashboard Â· pwa
...
**Seeded logins.**
`admin@bhrakshak.in / Admin@123` Â· `dc.aizawl@â€¦/ District@123`
Â· `field.noney@â€¦ / Field@123` Â· `citizen@bhrakshak.in / Citizen@123`.

> All metrics shown are computed by code (`make data`,
> `ml/models/backtest.py`) â€” never hard-coded in UI.

## Open Questions / Next Features

_To fill during planning once the team reviews this audit._

- [ ] Decision: layer-C signal â€” fake (deterministic PSInSAR) vs real (LiCSAR bulk download + GBSAR AOI tiles)
- [ ] Decision: SMS provider â€” MSG91 (India) vs Twilio (intl) vs CM-specific gateway
- [ ] Decision: ESP32 firmware scope â€” soil + tilt + LoRaWAN vs only MQTT-WiFi
